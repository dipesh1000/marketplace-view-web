...
function MainContainerBuyer({ singleOrder, orderId, singleArray }) {
  ...
  return (
    <div className="MainOrderContainer">
      ...
      <RequirementDetails
        ...
        slug={singleOrder?.orders?.gig?.slug}
        orderId={orderId}
        ...
        <div key={index}>
          ...
          {item?.datatype === "delivery" && (
            <DeliveryShow
              ...
              seller={singleOrder?.seller}
              orderId={orderId}
              revision={singleOrder?.orders?.remaning_revision}
            />
          )}
