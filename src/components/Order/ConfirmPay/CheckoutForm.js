...
const CheckoutForm = ({orderId, slug, handleStatus, isSubmitting, offerId}) => {
	...
	let payUrl = offerId ? "/api/order/custom_request/pay" : "/api/order/pay";

	const history = useHistory();
	const handleHistory = (orderId, slug) => {
		history.replace({
			pathname: `/checkout/submitrequirements`,
			state: {
				slug: slug,
				orderId: orderId,
			},
		...
		if (result.error) {
			...
			axiosInstance()
				.post(payUrl, {
					...
					custom_seller_offer_id: offerId ? offerId : null,
				})
	...
	const handleServerError = err => {
		...
		handleStatus(false);
		// handleStatus();
		// history.push(`/checkout/payments/${orderId}`);
	...
	return paymentMethod ? (
		...
		<>
			<form className='Form' onSubmit={handleSubmit} id='stripe-form'>
				{orderData?.order_code && <CardSection setError={setError} />}
...
const App = ({orderId, slug, handleStatus, isSubmitting}) => {
		...
		<div className='AppWrapper'>
			<Elements stripe={stripePromise} options={ELEMENTS_OPTIONS}>
				<CheckoutForm
					orderId={orderId}
					slug={slug}
					offerId={location?.state?.custom_seller_offer_id}
