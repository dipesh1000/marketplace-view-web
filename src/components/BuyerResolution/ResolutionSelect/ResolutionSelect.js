...
import { cancelOrderBuyer, fetchBuyerResolution } from "../redux/Action";
...
function BuyerResolutionSelect() {
  ...
  const { resolutions, isLoading } = useSelector(
  ...
  const handleCancelOrder = () => {
    setShowModal(false);
    dispatch(cancelOrderBuyer(orderId, handleHistory));
  };
  ...
  const handleRoute = () => {
    let pathname;
    // if (optionType == "buyer_progress_update") {
    //   pathname = `/users/dashboard/resolution/progress/${orderId}`;
    // } else if (optionType == "buyer_cancel_order") {
    //   pathname = `/users/dashboard/resolution/cancel/${orderId}`;
    // }
    switch (true) {
      ...
      case optionType === "buyer_progress_update" && optionSelect != null:
        pathname = `/users/dashboard/resolution/progress/${orderId}`;
        break;

      case optionType === "buyer_cancel_order" && optionSelect !== null:
        pathname = `/users/dashboard/resolution/cancel/${orderId}`;
        break;
            ...
            {!(
              ...
              <Row>
                <Col md={{ span: 8, offset: 2 }}>
                  ...
                  <div className="resolution-header"></div>
...
export default BuyerResolutionSelect;
